    ...
    <div class="loadmore-inner" :class="{'dropped': touchEnd}" :style="{ 'transform': 'translate3d(0, ' + translate + 'px, 0)' }">
      <slot name="top">
        <div class="loadmore-top" v-if="pulldown">
          <vui-spinner type="spinner2" v-if="topStatus === 'loading'"></vui-spinner>
          <vui-icon type="arrow-down" v-else></vui-icon>
      ...
      <slot name="bottom">
        <div class="loadmore-bottom" v-if="pullup">
          <vui-spinner type="spinner2" v-if="bottomStatus === 'loading'"></vui-spinner>
          <vui-icon type="arrow-up" v-else></vui-icon>
...
export default {
  ...
  watch: {
    pulldownStatus(val) {
      console.log('pulldownStatus', val)
      if (this.topStatus === STATUS_LOADING && val === STATUS_IDLE) {
        this.translate = 0
      }
      this.topStatus = val     
    },
    pullupStatus(val) {
      console.log('pullupStatus', val)
      if (this.bottomStatus === STATUS_LOADING && val === STATUS_IDLE) {
        this.translate = 0
      }
  ...
  methods: {
    ...
    handleTouchStart(event) {
      ...
      if (this.topStatus !== STATUS_LOADING) {
        this.topStatus = STATUS_IDLE;
      }
      if (this.bottomStatus !== STATUS_LOADING) {
